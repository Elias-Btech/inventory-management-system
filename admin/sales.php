            ...
            if (!empty($products)) {
                try {
                    ...
                    if (!empty($sale_items)) {
                        ...
                        // Insert sale record
                        $stmt = $conn->prepare("INSERT INTO sales (sale_number, customer_name, customer_phone, customer_email, subtotal, discount_percentage, discount_amount, tax_percentage, tax_amount, final_amount, payment_method, notes, sale_date, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)");
...
$sales = [];
try {
    $query = "SELECT s.*, u.username as created_by_name FROM sales s LEFT JOIN users u ON s.user_id = u.id WHERE $where_clause ORDER BY s.sale_date DESC LIMIT 50";
