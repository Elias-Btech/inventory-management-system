    ...
    if (empty($products)) {
        ...
        if (empty($error_message)) {
            if (empty($valid_items)) {
                ...
                try {
                    // Insert sale
                    $sale_stmt = $conn->prepare("INSERT INTO sales (sale_number, customer_name, customer_email, customer_phone, payment_method, subtotal, discount_amount, tax_amount, final_amount, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    ...
                    $get_qty_stmt = $conn->prepare("SELECT quantity FROM products WHERE id = ?");
                    ...
                    foreach ($valid_items as $item) {
                        ...
                        $get_qty_stmt->bind_param("i", $item['product_id']);
                        $get_qty_stmt->execute();
                        $prev_qty = $get_qty_stmt->get_result()->fetch_assoc()['quantity'];
                        ...
                        $trans_stmt = $conn->prepare("INSERT INTO stock_transactions (product_id, user_id, transaction_type, quantity, previous_quantity, new_quantity, notes) VALUES (?, ?, 'out', ?, ?, ?, ?)");
                        $trans_stmt->bind_param("iiiiis", $item['product_id'], $_SESSION['user_id'], $item['quantity'], $prev_qty, $new_qty, $transaction_notes);
                        $trans_stmt->execute();
                    }
