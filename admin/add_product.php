...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($error_message)) {
        if (!empty($name) && $price >= 0 && $quantity >= 0) {
            ...
            if (empty($error_message)) {
                $stmt = $conn->prepare("INSERT INTO products (name, sku, description, category_id, supplier_id, price, quantity, min_stock_level, image, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->bind_param("sssiiidiiss", $name, $sku, $description, $category_id, $supplier_id, $price, $quantity, $min_stock_level, $image, $status);
