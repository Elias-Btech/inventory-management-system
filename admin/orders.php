            ...
            if (!empty($products)) {
                try {
                    ...
                    if (!empty($order_items)) {
                        ...
                        // Insert order record
                        $stmt = $conn->prepare("INSERT INTO orders (order_number, customer_name, customer_email, customer_phone, customer_address, subtotal, total_amount, status, notes, order_date, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, NOW(), ?)");
...
$orders = [];
try {
    $query = "SELECT o.*, u.username as created_by_name FROM orders o LEFT JOIN users u ON o.user_id = u.id WHERE $where_clause ORDER BY o.order_date DESC LIMIT 50";
